...
export async function runMigration() {
  ...
  console.log(`\n🚀 Starting database migration for [${isPostgres ? 'PostgreSQL (Cloud)' : 'SQLite (Local)'}]...`);
  ...
  if (isPostgres) {
    ...
    try {
      ...
      await client.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_pg_refresh_tokens_user ON refresh_tokens(user_id);
        CREATE INDEX IF NOT EXISTS idx_pg_refresh_tokens_hash ON refresh_tokens(token_hash);
        CREATE INDEX IF NOT EXISTS idx_pg_courses_status ON courses(status);
        CREATE INDEX IF NOT EXISTS idx_pg_courses_instructor ON courses(instructor_id);
        CREATE INDEX IF NOT EXISTS idx_pg_enrollments_student ON enrollments(student_id);
        CREATE INDEX IF NOT EXISTS idx_pg_enrollments_course ON enrollments(course_id);
        CREATE INDEX IF NOT EXISTS idx_pg_submissions_assignment ON submissions(assignment_id);
        CREATE INDEX IF NOT EXISTS idx_pg_submissions_student ON submissions(student_id);
        CREATE INDEX IF NOT EXISTS idx_pg_attendance_course_date ON attendance_records(course_id, lecture_date);
        CREATE INDEX IF NOT EXISTS idx_pg_audit_logs_timestamp ON audit_logs(timestamp);
      `);

      await client.query('COMMIT');
      console.log('✅ PostgreSQL Schema Migration successfully completed.');
    } catch (err) {
    ...
    `);
    console.log('✅ SQLite Schema Migration successfully completed.');
  }
}
